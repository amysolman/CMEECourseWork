...
species_octaves_list[1:80] <- NULL
...
for (i in 2:length(octave_500[[i]])) {
  y <- octave_500[[1]][i+1]
  y <- as.numeric(y[[1]])
  x <- sum_vect(x, y)
...
oct_500_total <- x/(length(species_octaves_list)) #divide the totalled octave values by the number of octave values
