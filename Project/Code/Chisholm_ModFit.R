...
for(K in 1:max(length(data$species_rich))) {
  ...
  file_name = paste("../Results/results_", x[[2]][[1]], ".csv", sep="")
  ...
  p <- ggplot(data, aes(x=area, y=species_rich)) +
    ...
    annotate("text",x=(max(data$area)*0.7),y=(max(data$species_rich)*0.5),hjust=.2,label=paste("R-Squared", format(round(results_list[[j]][[1]], 2), nsmall = 2))) +
    annotate("text",x=(max(data$area)*0.7),y=(max(data$species_rich)*0.4),hjust=.2,label=paste("Theta", format(round(results_list[[j]][[2]], 2), nsmall = 2))) +
    annotate("text",x=(max(data$area)*0.7),y=(max(data$species_rich)*0.3),hjust=.2,label=paste("Immigration", format(round(results_list[[j]][[3]], 2), nsmall = 2))) +
    annotate("text",x=(max(data$area)*0.7),y=(max(data$species_rich)*0.2),hjust=.2,label=paste("Niches", results_list[[j]][[4]]))
  ...
  file_name = paste("../Results/plot_", x[[2]][[1]], ".pdf", sep="")
  pdf(file_name)
  print(plots[[j]])
  dev.off()
