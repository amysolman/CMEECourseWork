...
for (j in 1:length(datasets)) {
  ...
  #best_plot_points <- NA #set best points to NA
  #best_fit_results <- NA #set best fit results to NA
...
for(i in 1:max(length(data$species_rich))) {
  ...
  #store these results
  results <- data.frame(RSq, coef(chisholm_fit)["theta"], coef(chisholm_fit)["m0"], i)
  ...
  if (RSq > a) {
    best_fit_results[j][i] <- results
    best_plot_points[j][i] <- points_plot
    a <- RSq 
  }
