...
indices <- function(m, t_max, nu) {
  ...
  do_speciation = nu
  do_migration = m
  index_list <- list(Meta, Island, do_speciation, do_migration)
  names(index_list) <- c("Meta", "Island", "do_speciation", "do_migration")
  return(index_list)
...
simulation_one_timestep <- function(i) {
  ...
  if (num <= do_speciation) {
    Island[sample(length(Island), 1, replace = T)] <<- sample(100:500, 1) #give unique species at each speciation event
  } else if (num <= do_speciation + do_migration) {
    Island[sample(length(Island), 1, replace = T)] <<- Meta[sample(length(Meta), 1, replace = T)]
  } else {
    Island[sample(length(Island), 1, replace = T)] <<- Island[sample(length(Island), 1, replace = T)]
  }
