#Script to perform NLLS fitting on Simulation data
...
#library("LambertW") #for starting values when that happens
...
NLLSfits <- function() {
  ...
  theta_start <- 100000
  m0_start <- 0.05*sqrt(median(Data2$area))
  ...
  for (K in 1:ceiling(max(Data2$species))) { #for num of niches from 1 to max number of species
    ...
    fit_chisholm <- try(nlsLM(species ~ chisholm_model(area, theta, m0, K), data = Data2, 
                              start = list(theta = theta_start, m0 = m0_start)), silent = T)
...
#plot simulation, analytic and NLLS results
  ...
  p1 <- ggplot(Data, aes(x = area, y = species, group = SimAnalyticNLLS)) +
    ...
    annotate(geom="text", x=200, y=190, label=c(round(k, 2)),
             color="red") +
    annotate(geom="text", x=100, y=170, label="theta:",
             color="red") +
    annotate(geom="text", x=250, y=170, label=c(round(theta, 2)),
             color="red") +
    annotate(geom="text", x=150, y=150, label="ACrit (red dotted line):",
             color="red") +
    annotate(geom="text", x=570, y=150, label=c(round(ACrit, 2)),
             color="red") +
    geom_vline(xintercept = ACrit, linetype="dotted", 
               color = "red", size=1.5) +
    theme_bw()
  
pdf("../../Results/Simulation2/NLLS/SimAnalyticNLLSfit.pdf")
print(p1)
dev.off()

#let's export our parameter estimations
parameters <- cbind.data.frame(migration, niches, theta)
write.csv(parameters, "../../Results/Simulation2/parameters.csv", row.names = FALSE)
