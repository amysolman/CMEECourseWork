...
cluster_run_function <- function(J_meta, nu, m_start, m_num, max_k_size, k_max, wall_time, output_file_name) {
  
  j <- 1
  
  Meta <<- metacommunity(J_meta, nu, k = k_max) #generate metacommunity for this simulation
  ...
  for (b in 1:m_num) {
    ...
    for (a in 1:k_max) { # give different numbers of niches
