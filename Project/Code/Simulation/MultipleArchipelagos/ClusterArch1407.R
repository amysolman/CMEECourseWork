...
cluster_run_function <- function(J_meta, nu, m, max_k_size, k, wall_time, output_file_name) {
  ...
  #make a loop that produces multiple archipelagos that are labelled!
  
  store_my_islands <- list()
  
  for (x in 1:max_k_size) { #give multiple islands with same k and m rate, but different SIZED niches
    
    my_islands <- my_island(m, k, k_size = x) 
    
    store_my_islands[[x]] <- my_islands
    
  }
  
  #this renames each island in the archepelago
  name <- rep(paste0("archipelago_",1), length(store_my_islands))
  names(store_my_islands) <- name
