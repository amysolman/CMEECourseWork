...
timestep_my_focal_island <- function(focal_island, k) {
  ...
  while (z <= k) {
    unpack_me(focal_island[[1]][[z]]) #upack the community for one niche
...
cluster_run_function <- function(J_meta, nu, m_start, m_num, k_start, k_num, max_k_size, wall_time, output_file_name) {
  
  j <- 1
  
  m_rates <- seq(from = m_start, by = 0.005, length.out = m_num) 
  
  num_niches <- seq(from = k_start, by = 5, length.out = k_num)
  
  Meta <<- metacommunity(J_meta, nu, k = max(num_niches)) #generate metacommunity for this simulation
  ...
  for (mr in 1:length(m_rates)) {
    

      for (nn in 1:length(num_niches)) { # give different numbers of niches

    
        for (x in 1:max_k_size) { #give multiple islands with same k and m rate, but different SIZED niches
         ...
         my_islands <- my_island(m, k = a, k_size = x) 
         
         store_my_islands[[z]] <- my_islands
