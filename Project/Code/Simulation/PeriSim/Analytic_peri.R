...
for (j in 1:10){
  ...
  data <- read.csv(paste0("../../../Data/SimulationMeansPeri", j, ".csv"))
  ...
  chisholm_model <- function(area, theta, m0, K, rho) {
    Js = area*rho
    J_stars = Js/K
    ms = m0/sqrt(area) #per capita immigration rate
  ...
  get_my_model_species <- function(data){
    ...
    for (i in 1:nrow(data)) {
      ...
      K = island$K_true
      m0 = island$m0_true
      model_species[[i]] <- round(chisholm_model(area, theta, m0, K, rho=1), digits = 2)
    }
  ...
  #save as pdf
  file_name = paste0("../../../Results/Simulation/AnalyticPlotPeri", j, ".pdf")
  pdf(file_name)
  print(p)
  dev.off()
  
  #save results table
  write.csv(data, paste0("../../../Data/MeanAnalyticPeri", j, ".csv"))
