...
coalescence_test <- function(nu, k)
{
  metaniches <- list()
...
metacommunity <- function(nu, k) {
  
  meta <- vector() #empty vector for storing the community
  
  metaniches <- coalescence_test(nu, k) #run coalescence test and get 
...
simulation_one_timestep <- function(i, k, depth) {
  ...
  do_migration <- m0/depth #the likelihood of an immigration event occuring on the whole island
...
timestep_my_focal_island <- function(focal_island, k, depth) {
    
  z <-sample(k, 1, replace=FALSE)
      
      unpack_me(focal_island[[1]][[z]]) #upack the community for that niche
      
      y <- simulation_one_timestep(i, k, depth) #do simulation on niche and store community in y
...
cluster_run_function <- function(nu, m0, num_islands, k, wall_time, output_file_name) {
  
  j <- 1
  
  depth = 1
  
  Meta <<- metacommunity(nu, k) #generate metacommunity for this simulation
  ...
  while (proc.time()[3] - ptm < wall_time*60){ 
      ...
      num_islands <- length(store_my_islands)
        ...
        if (j <= num_islands) { #if j is between 1 and number of islands
          ...
          store_my_islands[[j]] <- timestep_my_focal_island(store_my_islands[[j]], k, depth) #do one timestep on one island
          ...
          store_my_islands[[j]] <- timestep_my_focal_island(store_my_islands[[j]], k, depth) #do next timestep on first island 
        }
  ...
  save(store_my_islands, timesteps, total_time, nu, file = output_file_name)
