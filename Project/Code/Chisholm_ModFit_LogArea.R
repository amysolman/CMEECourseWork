...
for (j in 1:length(datasets)) {
  ...
  data <- data.frame(LogArea, species_rich) #bind into a new dataframe we will use for fitting
  names(data) <- c("LogArea", "species_rich") #rename the columns
...
for(K in 1:max(length(data$species_rich))) {
  ...
  chisholm_fit <- try(nlsLM(species_rich ~ chisholm_model(LogArea, theta, m0, K), data = data, start = list(theta = 200, m0 = 0.0005, K=K)), silent=T)
  ...
  species <- chisholm_model(LogArea = LogArea, theta = coef(chisholm_fit)["theta"], m0 = coef(chisholm_fit)["m0"], K)
  points_plot <- data.frame(LogArea, species)
  names(points_plot) <- c("LogArea", "species")
  ...
  file_name = paste("../Results/LogAreaRsults_", x[[2]][[1]], ".csv", sep="")
  ...
  p <- ggplot(data, aes(x=LogArea, y=species_rich)) +
    geom_point() +
    xlab("log Area") +
    ...
    geom_line(data = best_plot_points, aes(x = LogArea, y = species)) +
    annotate("text",x=(max(data$LogArea)*0.7),y=(max(data$species_rich)*0.5),hjust=.2,label=paste("R-Squared", format(round(results_list[[j]][[1]], 2), nsmall = 2))) +
    annotate("text",x=(max(data$LogArea)*0.7),y=(max(data$species_rich)*0.4),hjust=.2,label=paste("Theta", format(round(results_list[[j]][[2]], 2), nsmall = 2))) +
    annotate("text",x=(max(data$LogArea)*0.7),y=(max(data$species_rich)*0.3),hjust=.2,label=paste("Immigration", format(round(results_list[[j]][[3]], 2), nsmall = 2))) +
    annotate("text",x=(max(data$LogArea)*0.7),y=(max(data$species_rich)*0.2),hjust=.2,label=paste("Niches", results_list[[j]][[4]]))
  ...
  file_name = paste("../Results/LogAreaPlot_", x[[2]][[1]], ".pdf", sep="")
  pdf(file_name)
  print(plots[[j]])
  dev.off()
