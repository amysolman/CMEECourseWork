...
for (ID in 1:length(unique(MyData$dataset_ID))){
  ...
  p1 <-ggplot(Data, aes(x=area, y = species)) +
    ...
    xlab(paste("log(area mm2)")) +
    ylab(paste("OTUs")) +
    geom_vline(xintercept = ACrit, linetype="dotted",
               color = "red", size=1.5)+
    theme_bw()

  file_name = paste("../../Results/ModelFit/NLLSplot_", ID, ".pdf", sep="")
  pdf(file_name)
  print(p1)
  dev.off()
...
write.csv(final_parameters, "../../Results/ModelFit/NLLSParameters.csv", row.names = FALSE)
