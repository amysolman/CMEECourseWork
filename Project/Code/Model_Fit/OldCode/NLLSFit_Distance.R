...
#distance metric (see other scripts for those with volume/area metrics)
...
#this gives us the datasets that have distance data only
...
get_my_points <- function() {
  ...
  distance <- Data$area
  chisholm_points <- chisholm_model(area = distance, theta = a[[1]], m0 = a[[2]], K = niches)
  
  #NLLS points into dataframe
  df <- data.frame(distance, chisholm_points)
  df <- round(df, 5)
  names(df) <- c("distance", "species")
  return(df)
  
}

all_parameters <- data.frame(ID = numeric(0), RSquared = numeric(0), criticaldistance = numeric(0),
                             m0 = numeric(0), mrate = numeric(0), k = numeric(0), theta=numeric(0))
names(all_parameters) <- c("ID", "R^2", "CriticalDistance", "m0", "m_rate", "k", "theta")
...
for (i in 1:length(unique(MyDistanceData$ID))) {
  ...
  ID <- unique(MyDistanceData$ID)[i] #get the ID
  
  Data<-MyDistanceData[which(MyDistanceData$ID==ID),]
  ...
  if(class(best_fit_and_k) !="try-error"){
    ...
    p1 <- ggplot(Data, aes(x = area, y = sp.rich)) +
      ...
      geom_smooth(data = df, aes(x=distance, y=species), se=F)+
    ...
    names(parameters) <- c("ID", "R^2", "CriticalDistance", "m0", "m_rate", "k", "theta")
