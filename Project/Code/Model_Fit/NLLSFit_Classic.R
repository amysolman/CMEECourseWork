...
#######################################################
#######FUNCTION FIVE: THE POWER LAW MODEL##############
#######################################################
...
for (i in 1:length(unique(MyData$datasetID))){
  ...
  #this interfers with the NLLS fitting
  ...
  #export plot
  file_name = paste0("../../Results/ClassicNLLSPlot", i, ".pdf")
  pdf(file_name)
  print(p)
  dev.off()
...
write.csv(MyData, "../../Data/ClassicACrit.csv")
write.csv(final_results, "../../Results/ClassicNLLSResults.csv", row.names = FALSE)
write.csv(Pow_final_results, "../../Results/ClassicPowResults.csv", row.names = FALSE)
