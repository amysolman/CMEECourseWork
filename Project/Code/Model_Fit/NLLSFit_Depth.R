...
NLLSfits <- function(Data) {
  ...
  for (K in 1:ceiling(max(Data$species))) { #for num of niches from 1 to max number of species
    ...
    #fit the model
    fit_chisholm <- try(nlsLM(species ~ chisholm_model(area, theta, m0, K, rho = round(Data$rho[[1]]), depth), data = Data, 
...
#################################################################
#############FUNCTION FIVE: CRITICAL AREA LOOP#####################
#################################################################

#Do the critical area fitting for each value of depth and plot
MultiCriticalArea <- function(theta, m0, K, rho, depths){
  my_critical_areas <- vector()
  for (i in 1:length(depths)){
    my_critical_areas[[i]] <- CriticalArea(theta, m0, K, rho, depth = depths[[i]])
  }
  return(my_critical_areas)
}

#######################################################
####FUNCTION SIZE: THE POWER LAW MODEL#################
#######################################################
...
for (i in 1:length(unique(MyData$datasetID))){
  ...
  #for some reason this interfers with the NLLS fitting
    ...
    rho = round(Data$rho[[1]])
    depth = Data$depth
    chisholm_points <- chisholm_model(Area, theta_best, m0_best, K_best, rho, depth)
    ...
    if (dim(table(Data[,36])) == 1){ #if all values in the depth column are the same
      ACrit <- CriticalArea(theta = ceiling(theta_best), m0 = m0_best, K = K_best, rho = rho, depth = Data$depth[[1]])
    ...
    #export plot
    file_name = paste0("../../Results/DepthNLLSPlot", i, ".pdf")
    pdf(file_name)
    print(p)
    dev.off()
      ...
      #if all values in the depth column are NOT the same
      ACritScores <- MultiCriticalArea(theta = ceiling(theta_best), m0 = m0_best, K = K_best, rho=rho, depths = Data$depth)
      
      #Plot the ACritScores and the depths and species richnesses 
      df2 <- data.frame(Area, depth, ACritScores, Data$species) 
      
      p <- ggplot(df2, aes(x=depth, y=log(ACritScores)))+
        geom_line(aes(x=depth, y=log(ACritScores), color="red"), size=3)+
        geom_point(aes(x=depth, y=log(Area), size=Data.species))+
        ylab("log Area (cm2)")+
        xlab("Depth (cm)")+
        #theme_bw()+
      ...
      df2$model.species <- df[df$categories == "NLLS", ]$plot_species
      df2$power.species <- df[df$categories == "Power", ]$plot_species
      
      p2 <- ggplot(df2, aes(x=depth*Area, y=Data.species))+
        geom_smooth(aes(x=log(depth*Area), y=model.species), method='lm',formula=y ~ poly(x, 3),se=F)+
        geom_smooth(aes(x=log(depth*Area), y=power.species), color="green", method='lm',formula=y ~ poly(x, 3),se=F)+
        geom_point(aes(x=log(depth*Area), y=Data.species))+
        geom_vline(xintercept = log(mean(df2$ACritScores)*mean(df2$depth)), linetype="dotted",
                   color = "red", size=3)+
        ylab("Species")+
        xlab("log Volume (Depth x Area)")+
        labs(size = "Species Richness")+
        guides(colour=FALSE)+
        theme(
          plot.title = element_blank(),
      ...
      #export plot
      file_name = paste0("../../Results/DepthNLLSPlot2_", i, ".pdf")
      pdf(file_name)
      print(p2)
      dev.off()
        
      ACrit <-mean(ACritScores)
    ...
    #fit the model
    fit_chisholm <- try(nlsLM(species ~ chisholm_model(area, theta, m0, K = K_best, rho = round(Data$rho[[1]]), depth), data = Data, 
                              start = list(theta = starts$theta_start, m0 = starts$m0_start), lower=c(0, 0), upper=c(Inf, 1)), silent = T)
...
write.csv(MyData, "../../Data/DepthACrit.csv")
write.csv(final_results, "../../Results/DepthNLLSResults.csv", row.names = FALSE)
write.csv(Pow_final_results, "../../Results/DepthPowResults.csv", row.names = FALSE)
