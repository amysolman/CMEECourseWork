...
folder <- "../../Data/Solman2020/Modified/"  # path to folder that holds multiple .csv files
...
for (j in 1:length(datasets)) {
  ...
  immigration_rate <- log(x[4]) #assign the area column of our dataframe to a vector
  cell_count <- x[5] #assign the species richness column of our dataframe to a vector
  ...
  date <- x[2]
  data <- data.frame(data, date[[1]][[1]])
  names(data) <- c("Immigration_Rate", "Cell_Count", "Date") #rename the columns
  ...
  #make immigration rate equivalent to area and cell count equivalent to species richness
  data$area <- data$Immigration_Rate
  data$species_rich <- data$Cell_Count
  ...
  file_name = paste("../Results/SingleRunResults_Solman2020_", data[[3]][[1]], ".csv", sep="")
